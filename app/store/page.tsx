...
export default async function Page({ searchParams }: PageProps) {
    ...
    return (
        <>
            <div id='storeContainer' className='h-full w-11/12 flex flex-col mx-auto pt-16 pb-4'>
                <SearchBar className={'p-2 lg:p-4 hidden sm:inline-flex'} />
                ...
                <div id="filtersAndItems" className="flex-1 sm:flex-none overflow-auto no-scrollbar flex relative flex-row max-w-full border-t-1 border-foreground">
                    
                    <div className="relative hidden sm:inline-block w-1/4 px-2 pt-1">
                        <FiltersPanel itemtype={category} type={'desktop'} />
                    </div>
                    <div className='inline-flex sm:hidden'><CategorySwitch /></div>
                    <ItemsPanel items={items} images={images.blobs} />
                </div>
                <SearchBar className={'p-4 inline-flex sm:hidden'} />
            </div>
        </>
    )
}