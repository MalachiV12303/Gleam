...
export function Cart() {
    ...
    return (
        <Popover isOpen={isOpen} onOpenChange={(open) => setIsOpen(open)} shouldBlockScroll={true} placement='bottom-end' backdrop='opaque' classNames={{
            trigger: ['border-1 border-foreground bg-transparent text-foreground', 'rounded-full h-7 min-w-12 sm:min-w-20'],
            content: ['border-1 border-foreground bg-background text-foreground text-lg lg:text-sm px-0 py-0', 'flex flex-row', 'max-h-[70dvh] h-[60dvh] w-[70dvw] sm:w-[55dvw] lg:w-[45dvw] xl:w-[35dvw]'],
        }}>
            <PopoverTrigger>
                <Button>
            ...
            <PopoverContent>
                ...
                <div id='cartButtons' className='h-full flex flex-col gap-2'>
                    <Tooltip
                        content='close'
                        classNames={{
