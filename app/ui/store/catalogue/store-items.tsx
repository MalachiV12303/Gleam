...
export function StoreItem({ item, image }: { item: Camera | Lense, image: ListBlobResultBlob | null }) {
            ...
            <>
                <div className='text-sm px-4 py-4 flex flex-col lg:flex-row gap-4 items-center justify-between overflow-hidden'>
                    ...
                    <div className='flex gap-2 items-center sm:justify-between'>
                        <div id='imagePlaceholder' className='border-1 border-foreground w-[150px] h-[150px] sm:w-[175px] sm:h-[175px] md:w-[225px] md:h-[225px] 2xl:w-[300px] 2xl:h-[300px] flex justify-center items-center'>
                            {image ?
                                ...
                                <img
                                    ...
                                    src={image.url}
                                    alt='image'
                                    className='max-w-[100px] max-h-[100px] sm:max-w-[125px] sm:max-h-[125px] md:max-w-[175px] md:max-h-[175px] 2xl:max-w-[200px] 2xl:max-h-[200px] w-auto h-auto'
    ...
    function Lense(len: Lense) {
        ...
        return (
            <>
                <div className='text-sm px-4 py-4 flex flex-col lg:flex-row gap-4 items-center justify-between overflow-hidden'>
                    <Link className='flex flex-col items-center sm:items-start' href={`/details?${params}`} >
                        <p className='text-xl text-center sm:text-start '>{len.name}</p>
                        <div className='lowercase flex gap-2 xl:gap-3'>
                            <p>{len.type}</p>
                            <p>{len.brand}</p>
                        </div>
                        <div className='flex gap-2 xl:gap-3'>
                            {len.minfl === len.maxfl ?
                                <p>min/max {len.minfl}mm</p> :
                                <>
                                    <p>min {len.minfl}mm</p>
                                    <p>max {len.maxfl}mm</p>
                                </>
                            }
                        </div>
                        <div className='lowercase flex gap-2'>
                            {len.mount?.map((mount, index) => (<p key={index}>{mount}</p>))}</div>
                    </Link>
                    <div className='flex gap-2 items-center sm:justify-between'>
                        <div id='imagePlaceholder' className='border-1 border-foreground w-[150px] h-[150px] sm:w-[175px] sm:h-[175px] md:w-[225px] md:h-[225px] 2xl:w-[250px] 2xl:h-[250px] flex justify-center items-center'>
                            {image ?
                                // eslint-disable-next-line @next/next/no-img-element
                                <img
                                    key={item.id}
