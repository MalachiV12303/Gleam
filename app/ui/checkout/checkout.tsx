...
export function Checkout() {
    ...
    return (
        <>
            <div className='max-w-[1400px] flex flex-1 scrollbar overflow-auto overflow-x-hidden w-full text-background'>
                <Accordion className='px-0' defaultExpandedKeys={['cart', 'payment', 'tools']} itemClasses={{ content: 'text-foreground bg-background py-8 px-4', title: 'text-background text-lg', indicator: 'text-background', trigger: 'mt-4 bg-foreground text-background px-4' }} selectionMode={'multiple'}>
                    ...
                    <AccordionItem key='payment' aria-label='billing and payment' title='billing and payment'>
                        <form ref={constraintsRef2} className='flex sm:flex-row flex-col items-center justify-between gap-8 '>
                            <motion.div drag dragConstraints={constraintsRef2} className='flex-1 flex flex-col gap-2'>
                                <p>PERSONAL INFO</p>
                            ...
                            <motion.div drag dragConstraints={constraintsRef2} className='flex-1 flex items-start sm:items-end flex-col gap-2'>
                                <p>CARD INFO</p>
                    ...
                    <AccordionItem key='tools' aria-label='tools used' title='tools used'>
                        <div ref={constraintsRef} className='min-h-56 text-sm sm:text-base text-background grid grid-cols-3 gap-y-2 sm:gap-y-8 gap-x-8'>
                            {tools.map((tool, index) => (
                                <motion.div dragTransition={{ bounceStiffness: 100, bounceDamping: 10 }} drag dragConstraints={constraintsRef} key={index} className='w-[50%] border-black bg-foreground flex items-center justify-center border-1 py-2 rounded-lg uppercase tracking-wider'>
                                    {tool}
                                </motion.div>
                            ))}
                        </div>
            ...
            <div className='mx-2 mt-2 py-4 select-none flex flex-col w-full px-8'>
